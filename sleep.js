...
class ClientAPI {
  ...
  async handleUpgradeHeroes(data, goldAvailableInit = null, greenStonesAvailableInit = null) {
    this.log(`Checking upgrade hero...`);
    let { heroes, resources } = data;
    const { greenStones, gold, heroCard } = resources;
    let goldAvailable = goldAvailableInit || gold.amount,
      greenStonesAvailable = greenStonesAvailableInit || greenStones.amount;
    ...
    // }
    // return data;
    return await this.handleUpgradeHeroes(data, goldAvailable, greenStonesAvailable);
  }
